...
function findLastEvenNumber(arr) {
  ...
  const lastEvent = arr.findLast((obj) => obj % 2 === 0);
  return lastEvent;
}
...
function getArrayKeys(arr) {
  ...
  const arrKeys = Array.from(getKeys);
...
function removeAndDouble(arr, startIdx) {
  ...
  // Замість видалення конкретної кількості елементів, видаляємо всі елементи після вказаного індексу.
  const removedElements = arr.splice(startIdx);

  // Подвоюємо залишені елементи.
...
function wordLengths(arr) {
  ...
  const newLenghts = arr.map((wo) => wo.length);
...
function expandArrayByN(arr, n) {
  ...
  const arrBy = arr.flatMap((dub) => Array(n).fill(dub));
...
function findLongestWord(arr) {
  ...
  // const longer = "";
  const longerWord = arr.reduce(
    (accum, currVal) => (currVal.length > accum.length ? currVal : accum),
    ""
  );

  // const longerWord = arr.reduce((longest, currentWord) =>
...
function findDuplicateElements(arr) {
  ...
  const duplicateElements = arr.filter((value, index, self) => {
...
function findLastNegativeNumberIndex(arr) {
  ...
  return lastIndex !== -1 ? arr.length - 1 - lastIndex : -1;
}
